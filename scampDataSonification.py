...
def constructData():
    global tracks
    import umap
    reducer = umap.UMAP(n_neighbors=15,min_dist=0.1,n_components=len(tracks),metric="euclidean",random_state=42)
    ...
    mydata = pd.read_csv("cumulative.csv",sep=",",header=0)
...
def scamp_loop():
   global countBass,tracks,oneOctave,SYMFUNC,NFUNC,BASEFUNC, counters, listFuncs,number_of_counters_x,s,started_transcribing, data, index, session_finished
